      ...
      {days.map((day, i) => {
        ...
        return (
          <div key={i} className="flex flex-col gap-1 items-center group relative h-full justify-end w-full">
             {/* Tooltip */}
             <div className="hidden md:block absolute -top-12 left-1/2 -translate-x-1/2 bg-black text-white text-[9px] font-bold px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-20 pointer-events-none">
             ...
             <motion.div 
               ...
               className={`w-full rounded-sm md:rounded-md ${color} transition-colors duration-300`}
             />
          </div>
        )
      })}
...
export default function Analytics() {
  ...
  return (
    <MainLayout>
      <PageTransition>
        <div className="w-full min-h-screen bg-[#F1F3F5] p-4 md:p-6 lg:p-8 font-sans text-slate-900 pb-24 lg:pb-8">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 md:gap-6">
            <div>
              <h1 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tight flex items-center gap-3">
                 Analytics <span className="text-gray-300 hidden md:inline">/</span> <span className="text-gray-400 font-medium text-lg hidden md:inline">Insight Engine</span>
              </h1>
            </div>
            <div className="flex w-full md:w-auto bg-white p-1 rounded-2xl border border-gray-200 shadow-sm">
               {['7d', '30d', '90d'].map((r) => (
                  <button key={r} onClick={() => setTimeRange(r as any)} className={`flex-1 md:flex-none px-6 py-2 rounded-xl text-xs font-black uppercase transition-all ${timeRange === r ? 'bg-black text-[#A9FF53] shadow-md' : 'text-gray-400 hover:text-black'}`}>
                     {r}
                  </button>
               ))}
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6">
             ...
             <div className="col-span-1 lg:col-span-8 bg-black text-white rounded-[2.5rem] p-6 md:p-10 relative overflow-hidden shadow-2xl shadow-black/20 group min-h-[350px] md:h-[400px] flex flex-col justify-between border border-white/5">
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start gap-4">
                   <div>
                      ...
                      <h2 className="text-5xl md:text-7xl font-black tracking-tighter text-white mb-2">
                         {formatCurrency(totalSpend, currency).replace(currency, '')}<span className="text-3xl md:text-4xl text-white/30">{currency}</span>
                      </h2>
                ...
                {/* Area Chart */}
                <div className="relative z-10 h-32 md:h-40 w-full mt-auto translate-y-4">
             ...
             <div className="col-span-1 lg:col-span-4 flex flex-col gap-4 md:gap-6">
                ...
                {/* Heatmap Card */}
                <div className="bg-white rounded-[2.5rem] p-6 md:p-8 shadow-sm flex-1 flex flex-col justify-between border border-gray-100 relative overflow-hidden min-h-[200px]">
                ...
                {/* Insight Card */}
                <div className="bg-[#A9FF53] rounded-[2.5rem] p-6 md:p-8 shadow-xl shadow-[#A9FF53]/20 h-[180px] relative overflow-hidden group">
             ...
             <div className="col-span-1 lg:col-span-8 bg-white rounded-[2.5rem] p-6 md:p-8 shadow-sm min-h-[350px]">
             ...
             <div className="col-span-1 lg:col-span-4 bg-[#111] text-gray-400 rounded-[2.5rem] p-6 md:p-8 shadow-lg flex flex-col font-mono text-xs relative overflow-hidden">
